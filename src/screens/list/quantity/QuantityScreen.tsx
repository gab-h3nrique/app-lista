    ...
    <Animated.View style={[tw`w-full h-full flex absolute`, { transform: [{translateX: positionScreen}], }]}>
      ...
      <View style={tw`p-3 gap-30 w-full h-full bg-slate-200 dark:bg-slate-800 flex justify-start`}>
        ...
        <View style={tw`items-center justify-center flex flex-row w-full relative`}>
          ...
          <Button onPress={()=> navigate.close('QuantityScreen')} style={tw`left-0 top-2 w-9 h-8 rounded-[.6rem] bg-slate-400 dark:bg-slate-700 flex items-center justify-center absolute`} >
        ...
        <View style={tw`px-6 gap-5 mt-auto w-full flex`}>
          ...
          <Button disabled={!quantity} onPress={()=> {selectQuantity(quantity)}} style={tw`p-3 rounded-[.9rem] flex justify-center items-center bg-violet-400 ${quantity ? 'opacity-100' : 'opacity-50'}`}>

...
const EditListScreen = () => {
  ...
  function editSelectedList(list: List) {
    ...
    saveUserContext({ ...user, selectedList: updatedList })

  }

  function openEditItemScreen(item: Item) {

    navigate.open('EditItemScreen', { item })
  ...
  function editCheckItem(item: Item) {
    ...
    saveUserContext({ ...user, lists:Storage.List.getMany(), selectedList: updatedList })
  ...
  function removeItem(id: number) {
    ...
    if(!user.selectedList) return console.warn('this item was not found');

    saveUserContext({
      ...user, 
      selectedList: {
        ...user.selectedList, 
        itens:  user.selectedList?.itens.filter((e)=> e.id !== id),
      }
    })
  ...
  return (
    ...
    <View style={tw`w-full h-full bg-slate-200 dark:bg-slate-800 relative`}>
      ...
      <View style={tw`p-3 justify-center items-center flex flex-row w-full relative`}>
        ...
        <Button onPress={navigate.closeLast} style={tw`left-2 top-5 w-9 h-8 rounded-[.6rem] bg-slate-400 dark:bg-slate-700 flex items-center justify-center absolute`} >
      ...
      <View style={tw`px-4 gap-3 flex justify-start items-center`}>
        ...
        {
          user.selectedList && user.selectedList.itens.length > 0 ? (

            user.selectedList.itens.map(( item, i) =>
